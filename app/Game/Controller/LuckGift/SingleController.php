...
#[Controller(prefix: 'game/luck-gift/single-config')]
class SingleController extends MineController
{
    #[Inject]
    ...
    public function addLuckGiftPoolGift(MineFormRequest $request): ResponseInterface
    {
        $datas = $request->all() ?? [];
        $id = $datas['id'];
        return $this->service->addLuckGiftPoolGift($id) ? $this->success() : $this->error($this->service->getErrMsg());
    }
